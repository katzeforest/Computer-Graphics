			...
			{ // Hit: compute the pixel color
				...
				for( float distance = 0.0f; distance < maxD; distance += stepsize)
				{
					...
					if (voxelLight  < 0) // Light not processed yet
					{
						...
						float distolight = lightDir.length() + 2*stepsize; //Add stepsize to make sure not to miss the target voxel 
 						float Tlight = 1 ;
						...
						for(float d = 0; d < distolight; d += stepsize)
						{
							...
							{
								Tlight *= exp(-voxelData.m_densityBuffer[index]/voxelData.m_mostDense);
