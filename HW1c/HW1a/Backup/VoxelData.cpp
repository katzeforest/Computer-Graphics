			...
			for(int k =0; k < m_sizeZ; k++)
				{
					...
					if(dis <= mySphereRadius)
						m_densityBuffer[getVoxelID(i,j,k)] += 1.0f*(1.0-dis/mySphereRadius);   //Density is a linear value
				}
}
			...
			for(int k =0; k < m_sizeZ; k++)
				{
					...
					m_densityBuffer[getVoxelID(i,j,k)] += noiseDensitytemp;   
				}
			...
			for(int k =0; k < m_sizeZ; k++)
				{
					...
					m_densityBuffer[getVoxelID(i,j,k)] += pyroclasticDensitytemp*scale;   
				}
