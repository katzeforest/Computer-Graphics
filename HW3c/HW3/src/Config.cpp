...
Config::Config(void) : m_lightPosition(0,9,0,1)
{
	s_instance = this;
    m_lightColor[0] = 1;
    m_lightColor[1] = 1;
    ...
    while (std::getline(fileStream, line))
    {
		...
		if (line.length() >= 3 && !line.compare(0, 3, "MAT"))
		{
			...
			for (unsigned int i = 0; i < matParams; ++i)
			{
				...
				else if(!line.compare(0, 4, "EMIT"))// 1 = emitting light
				{
					valueStream >> emit;
					std::cout << "EMIT " << emit << std::endl;
				}
			}
			m_materials.insert( std::pair<std::string,Material*>(matName, new Material(diffuseColor, reflectiveColor, specularExponent, refractionIndex, mirror, trans, emit)) );
		}
