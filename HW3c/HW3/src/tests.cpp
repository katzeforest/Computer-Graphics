                             ...
                             mat4 const& T)
{
	...
	glm::vec3 P0t(inverseT * glm::vec4(P0,1));
	glm::vec3 V0t(inverseT * glm::vec4(V0,0));

	Intersection inter;

	Geometry::intersectTriangle(Ray(P0t, V0t), p1, p2, p3, &inter);
...
void RunRayPolyTests()
{
    ...
    RunTest(
        ...
        -1.0); // TODO change this to the right number
}
...
void RunRayCubeTests()
{
    ...
    RunTest(
        ...
        6.36396); // TODO change this to the right number
}
...
void RunRayCylinderTests()
{
    ...
    RunTest(
        ...
        0.5); //TODO change this to the right number
}
...
void RunYourTests()
{
	...
	RunTest(
        "Triangle on vertex",
		Test_RayPolyIntersect(POSZ_VECTOR, NEGZ_VECTOR, vec3(0.0f, 0.0f, 0.0f), vec3(1.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f), IDENTITY_MATRIX),
        1.0);

	RunTest(
        "Triangle on edge",
		Test_RayPolyIntersect(vec3(0.5f, 0.0f, 1.0f), NEGZ_VECTOR, vec3(0.0f, 0.0f, 0.0f), vec3(1.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f), IDENTITY_MATRIX),
        1.0);

	RunTest(
        "Cube rescaled",
        Test_RayCubeIntersect(vec3(-0.5f, 0.0f, 2.0f), NEGZ_VECTOR, DOUBLE_MATRIX),
        1.0);

	mat4 rotateZ45 = glm::rotate(mat4(), 0.78539816339f, vec3(0,0,1));

	RunTest(
        "Cube rotated miss",
        Test_RayCubeIntersect(vec3(0.0f, 0.0f, 1.0f), NEGZ_VECTOR, rotateZ45),
        -1.0);
}
