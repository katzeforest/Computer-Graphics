			...
			{ // Hit: compute the pixel color
				...
				for( float distance = 0.0f; distance < maxD; distance += stepsize)
				{
					...
					if (voxelLight  < 0) // Light not processed yet
					{
						...
						float distolight = lightDir.length() + stepsize; //Add stepsize to make sure not to miss the target voxel 
 						float Tlight = 1 ;
						...
						for(float d = 0; d < distolight; d += stepsize)
						{
							...
							{
								float lightValue = voxelData.m_lightBuffer[index];

								if (lightValue < 0)
									Tlight *= exp(-voxelData.m_densityBuffer[index]); // No previously computed value, compute light now
								else
									Tlight *= lightValue; // Use the already computed value
					...
					double Ttemp = exp(-k*stepsize*voxelDensity*100);
					T *= Ttemp;
				...
				PixelColor.Red = (ebmpBYTE) ((C[0] + m_bgcolor[0] * T)*255);
				PixelColor.Green = (ebmpBYTE) ((C[1] + m_bgcolor[1] * T)*255);
				PixelColor.Blue = (ebmpBYTE) ((C[2] + m_bgcolor[2] * T)*255);
				PixelColor.Alpha = 0;
